...
{
    public class XFrameworkXLuaRuntimeAsynchronousILuaTaskBridge : LuaBase, ILuaTask
    {
	    public static LuaBase __Create(int reference, LuaEnv luaenv)
		{
		    return new XFrameworkXLuaRuntimeAsynchronousILuaTaskBridge(reference, luaenv);
		}
		
		public XFrameworkXLuaRuntimeAsynchronousILuaTaskBridge(int reference, LuaEnv luaenv) : base(reference, luaenv)
        {
        }
		
        
		object ILuaTask.GetResult()
		{
		...
		string ILuaTask.GetException()
		{
            ...
            lock (luaEnv.luaEnvLock)
            {
				...
				LuaAPI.lua_getref(L, luaReference);
				LuaAPI.xlua_pushasciistring(L, "GetException");
				if (0 != LuaAPI.xlua_pgettable(L, -2))
				{
					luaEnv.ThrowExceptionFromError(err_func - 1);
				}
				if(!LuaAPI.lua_isfunction(L, -1))
				{
					LuaAPI.xlua_pushasciistring(L, "no such function GetException");
					luaEnv.ThrowExceptionFromError(err_func - 1);
				}
				LuaAPI.lua_pushvalue(L, -2);
				LuaAPI.lua_remove(L, -3);
		...
		void ILuaTask.OnCompleted(System.Action continuation)
		{
#if THREAD_SAFE || HOTFIX_ENABLE
            lock (luaEnv.luaEnvLock)
            {
