    ...
    public class TypeFinderUtils
    {
        ...
        private static async Task<List<Assembly>> GetAssemblies()
        {
            ...
            foreach (var file in files.Where(file => file.FileType == ".dll" || file.FileType == ".exe"))
            {
                try
                {
                    if (Regex.IsMatch(file.Name, "^((mscorlib)|(nunit)|(ucrtbased)|(Microsoft)|(ClrCompression)|(BridgeInterface)|(System)|(UnityEngine)|(UnityPlayer)|(XFramework.Log)|(WinRTLegacy))"))
                        continue;
        ...
        private static List<Assembly> GetAssemblies()
        {
            ...
            foreach (Assembly assembly in listAssembly)
            {
                ...
                if (Regex.IsMatch(name, "^((mscorlib)|(nunit)|(System)|(UnityEngine)|(XFramework.Runtime.Log))"))
                    continue;
