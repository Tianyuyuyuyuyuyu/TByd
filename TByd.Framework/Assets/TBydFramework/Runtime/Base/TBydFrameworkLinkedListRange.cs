...
namespace TBydFramework.Runtime.Base
{
    ...
    public struct TBydFrameworkLinkedListRange<T> : IEnumerable<T>, IEnumerable
    {
        ...
        public TBydFrameworkLinkedListRange(LinkedListNode<T> first, LinkedListNode<T> terminal)
        {
            if (first == null || terminal == null || first == terminal)
            {
                throw new TBydFrameworkException("Range is invalid.");
            }
        ...
        {
            private readonly TBydFrameworkLinkedListRange<T> _mBydFrameworkLinkedListRange;
            private LinkedListNode<T> m_Current;
            private T m_CurrentValue;

            internal Enumerator(TBydFrameworkLinkedListRange<T> range)
            {
                if (!range.IsValid)
                {
                    throw new TBydFrameworkException("Range is invalid.");
                ...
                _mBydFrameworkLinkedListRange = range;
                m_Current = _mBydFrameworkLinkedListRange.m_First;
            ...
            {
                if (m_Current == null || m_Current == _mBydFrameworkLinkedListRange.m_Terminal)
                {
                    return false;
                }
