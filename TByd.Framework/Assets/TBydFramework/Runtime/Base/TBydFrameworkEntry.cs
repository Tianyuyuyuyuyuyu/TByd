...
namespace TBydFramework.Runtime.Base
{
    ...
    /// 游戏框架入口。
    /// </summary>
    public static class TBydFrameworkEntry
    {
        private static readonly TBydFrameworkLinkedList<AbstractFrameworkModule> s_AbstractFrameworkModules = new TBydFrameworkLinkedList<AbstractFrameworkModule>();
        ...
        public static T GetModule<T>() where T : class
        {
            ...
            {
                throw new TBydFrameworkException(string.Format("You must get module by interface, but '{0}' is not.", interfaceType.FullName));
            }

            if (!interfaceType.FullName.StartsWith("GameFramework.", StringComparison.Ordinal))
            {
                throw new TBydFrameworkException(string.Format("You must get a Game Framework module, but '{0}' is not.", interfaceType.FullName));
            }
            ...
            if (moduleType == null)
            {
                throw new TBydFrameworkException(string.Format("Can not find Game Framework module type '{0}'.", moduleName));
            }
        ...
        private static AbstractFrameworkModule CreateModule(Type moduleType)
        {
            ...
            if (module == null)
            {
                throw new TBydFrameworkException(string.Format("Can not create module '{0}'.", moduleType.FullName));
            }
